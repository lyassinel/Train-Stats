    ...
    const loadStats = async () => {
        try {
            ...
            const [res1, res2] = await Promise.all([
                fetch(`/api/stats?depot=${filters1.depot || ''}&date=${filters1.date || ''}&serie=${filters1.serie || ''}`).then(r => r.json()),
                fetch(`/api/stats?depot=${filters2.depot || ''}&date=${filters2.date || ''}&serie=${filters2.serie || ''}`).then(r => r.json()),
            ])
            setStats1(res1)
            setStats2(res2)
        } catch (e) {
    ...
    return (
        <div className="p-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                {[filters1, filters2].map((f, idx) => (
                    <div key={idx} className="bg-gray-100 p-4 rounded">
                        ...
                        <select className="w-full mb-2 p-2" value={f.depot || ''} onChange={e => setFilters1({ ...f, depot: e.target.value })}>
                            <option value="">-- Dépôt --</option>
                            {depots.map(d => <option key={d} value={d}>{d}</option>)}
                        </select>
                        <select className="w-full mb-2 p-2" value={f.date || ''} onChange={e => setFilters1({ ...f, date: e.target.value })}>
                            <option value="">-- Date --</option>
                            {dates.map(d => <option key={d} value={d}>{d}</option>)}
                        </select>
                        <select className="w-full p-2" value={f.serie || ''} onChange={e => setFilters1({ ...f, serie: e.target.value })}>
            ...
            <div className="flex gap-2 mb-4">
                <button onClick={() => { updateURL(); loadStats() }} className="px-4 py-2 bg-blue-600 text-white rounded">Appliquer</button>
                <button onClick={exportCSV} className="px-4 py-2 bg-green-600 text-white rounded">Export CSV</button>
            </div>

            {/* Loading / Erreur */}
            {loading && <div className="flex items-center gap-2"><Loader2 className="animate-spin" /> Chargement...</div>}
